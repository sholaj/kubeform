...
type AwsConfigConfigurationRecorderStatusLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.AwsConfigConfigurationRecorderStatus, err error)
	// Get retrieves the AwsConfigConfigurationRecorderStatus from the index for a given name.
	Get(name string) (*v1alpha1.AwsConfigConfigurationRecorderStatus, error)
...
// List lists all AwsConfigConfigurationRecorderStatuses in the indexer.
func (s *awsConfigConfigurationRecorderStatusLister) List(selector labels.Selector) (ret []*v1alpha1.AwsConfigConfigurationRecorderStatus, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.AwsConfigConfigurationRecorderStatus))
	})
	return ret, err
}

// Get retrieves the AwsConfigConfigurationRecorderStatus from the index for a given name.
func (s *awsConfigConfigurationRecorderStatusLister) Get(name string) (*v1alpha1.AwsConfigConfigurationRecorderStatus, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("awsconfigconfigurationrecorderstatus"), name)
	}
	return obj.(*v1alpha1.AwsConfigConfigurationRecorderStatus), nil
}
